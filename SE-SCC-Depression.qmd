...
```{r}
#| label: loading-packages-and-libraries
#| echo: false
#| warning: false
#| message: false
#setting CRAN mirror
...
#loading clean data
wave1_cohort1_clean <- read_csv("Data/Wave1_Cohort1_Clean.csv")
#loading depression data
cesd_scored <- read_csv("Data/CESD_Scored.csv")
#loading explicit self esteem data
RSES_Scored <- read_csv("Data/RSES_Scored.csv")
#loading raw IAT data
wave1_cohort1_IAT <- read.csv("Data/Wave1_Cohort1_SelfIAT.csv", header=T)
#loading implicit self esteem data
selfIAT_scored <- read_csv("Data/SelfIATOutput.csv")
```
...
#combines four blocks of both practice and critical blocks of the IAT into a single variable one compatible and one incompatible
...
#| warning: false
#| message: false
#cleaning the IAT practice and critical blocks (in order of arguments given)
#drops trials that exceed a timeout threshold
#defines the timeout threshold in milliseconds
#does not drop fast trials
#drops participants who have too many fast trials
#too many fast trials defined as 0.10
#fast trial defined in milliseconds
#penalty given for incorrect responses
#the penatly added is in milliseconds
...
#| message: false
#| include: false
#calculating the internal consistency of the IAT scores using the custom function IATreliability
#returns crobach's alpha as a reliability score for the cleaned IAT data
IATreliability(clean)$reliability #internal consistency (0.8902007)
#calculating the reliability of the IAT's D-score using IATalpha
#returns raw alpha value to represent reliability
...
wave1_cohort1_clean <- wave1_cohort1_clean %>%
  ...
  ungroup()
#below is work done to verify total score actually populated, experienced significant issues with it in debugging, which is why it's in the same block
...
#comparing scores calculated in R and SPSS (previosly calculated in SPSS for lab) 
...
#below is work done to verify depression level actually populated, experienced significant issues with it in debugging, which is why it's in the same block
#printing to verify
...
#| echo: false
#creating table one showing depression and self-esteem relationship
  wave1_cohort1_clean %>%
    rename(`CES-D Total Score` = CESD_Total,
    `Depression Level` = Depression_Level,
    `Rosenberg Total Score` = Rosenberg_Total,
    `Explicit Self-Esteem Level` = Explicit_SE_Level) %>%
    select(`CES-D Total Score`, `Depression Level`, `Rosenberg Total Score`, `Explicit Self-Esteem Level`) %>%
  head() %>%
  flextable() %>%
  theme_apa() %>%
  add_footer_row(
...
#| echo: false
#creating table 2 showing a summary of rosenberg scores
  wave1_cohort1_clean %>%
    rename(`Depression Level` = Depression_Level,
    `Explicit Self-Esteem Level` = Explicit_SE_Level) %>%
    select(`Depression Level`, `Explicit Self-Esteem Level`) %>%
  head() %>%
  flextable() %>%
  theme_apa() %>%
  autofit()
...
#| warning: false
#| message: false
#graph of rosenberg score against depression level with facet
ggplot(wave1_cohort1_clean, aes(x = Depression_Level, y = Rosenberg_Total)) +
  geom_boxplot(fill = "lightgreen", color = "black", alpha = 0.8) +
  facet_grid(. ~ factor(Gender, levels = c(0, 1), labels = c("Female", "Male")), scales = "free_x") +
...
ggplot(wave1_cohort1_clean, aes(x = CESD_Total, y = Rosenberg_Total, color = Depression_Level)) +
  ...
  labs(x = "CES-D Total Score", y = "Rosenberg Total Score (Range: 0-30)", color = "Depression Level", caption = "Note: higher Rosenberg total scores indicate higher self-esteem.") +
  theme_minimal(base_size = 12) +
  theme(plot.title = element_text(size = 14, face = "bold", hjust = 0.5),
    axis.title.x = element_text(size = 12, face = "bold"),
    axis.title.y = element_text(size = 12, face = "bold"),
...
#| warning: false
#| message: false
#graph of disribution of CES-D scores by depression level with facet
ggplot(wave1_cohort1_clean, aes(x = CESD_Total)) +
...
#presented results in narrative text
```
...
#calculating further correlation tests but for ANOVA summary
cor_test_p_value <- cor_test_result$p.value
cor_test_estimate <- cor_test_result$estimate
cor_test_conf_int <- cor_test_result$conf.int
#calculating further linear regression but for ANOVA summary
lm_summary <- summary(lm_model)
lm_p_value <- lm_summary$coefficients[2, 4]
lm_beta <- lm_summary$coefficients[2, 1]
lm_conf_int <- confint(lm_model)[2, ]
...
```{r}
#| label: numeric-and-non-numeric-summary-statistics
#| echo: FALSE
#| warning: false
#| message: false
#| include: false
...
summary_stats <- wave1_cohort1_clean %>%
  summarise(
    ...
    SD_RSE = sd(Rosenberg_Total, na.rm = TRUE))
print(summary_stats)
#non-numeric numeric summary statistics
non_numeric_summary_stats <- wave1_cohort1_clean %>%
...
```{r}
#| label: tbl-numeric-summary-stats
#| tbl-cap: Descriptive Statistics for Numeric Variables
#| echo: false
#| warning: false
#| message: false
#table of numeric summary statistics
numeric_table <- summary_stats %>%
  rename(`Mean CES-D Score` = Mean_CESD,
    `Standard Deviation of CES-D Score` = SD_CESD,
    `Mean Rosenberg Self-Esteem Score` = Mean_RSE,
    `Standard Deviation of Rosenberg Self-Esteem Score` = SD_RSE) %>%
...
```{r}
#| label: tbl-non-numeric-summary-stats
#| tbl-cap: Frequency and Proportion of Depression Levels
#| echo: false
#| warning: false
#| message: false
#table of non-numeric summary statistics
non_numeric_table <- non_numeric_summary_stats %>%
  set_names(c("Depression Level", "Frequency", "Proportion")) %>%
  flextable() %>%
  theme_apa() %>%
  autofit()
...
The mean CES-D score was `r summary_stats$Mean_CESD` (*SD* = `r summary_stats$SD_CESD`). The mean Rosenberg score was `r summary_stats$Mean_RSE` (*SD* = `r summary_stats$SD_RSE`). An ANOVA was calculated to demonstrate the imapct of depression level on CES-D scores, results should be obvious since coming from the same measure (*F* = `r f_value`, *p* = `r p_value`, η² = `r eta_squared_value`). As anticipated and assumed, the effect size (η² = `r eta_squared_value`) shows a large effect of depression level on CES-D scores. A linear regression was perfomed which demonstrated that age `r ifelse(lm_p_value < 0.05, "was", "was not")` a significant predictor of the occurance of depressive symptoms (*β* = `r lm_beta`, *p* = `r lm_p_value`, 95% CI [`r lm_conf_int[1]`, `r lm_conf_int[2]`]). Finally, a correlation analyses was perfomed between the Rosenberg Self-Esteem scores and the CES-D scores, which portrayed a significant negative relationship (*r* = `r cor_test_estimate`, *p* = `r cor_test_p_value`, 95% CI [`r cor_test_conf_int[1]`, `r cor_test_conf_int[2]`]). This shows that lower depressive symptoms are associated with higher self-esteem.^[The CES-D is a scale that ranges from 0 to 60, with the higher scores indicating higher severity of depressive symptoms.]
The CES-D score distribution by depression level is visualized in Figure @ref(fig-CESD-score-distribution). The relationship between CES-D scores and Rosenberg Self-Esteem scores is shown in Figure @ref(fig-RSES-versus-CESD-total). Descriptive statistics for this sample's numeric variables are presented in Table @ref(tbl-numeric-summary-stats), and the proportion and frequency of the demonstrated depression levels in the CES-D are given in Table @ref(tbl-non-numeric-summary-stats).The variety of Rosenberg Self-Esteem scores present in the sample is shown in Figure @ref(fig-RSES-score-distribution), and the Implicit Association Test (IAT) scores are exhibited in Figure @ref(fig-IAT-score-distribution). The connection between CES-D depression levels and Rosenberg scores, stratified by gender, is portrayed in Figure @ref(fig-RSES-versus-CESD-level). A regression of CES-D total scores on age is represented in Figure @ref(fig-CESD-total-versus-age).The summary of depression scores with corresponding explicit self-esteem scores is given in Table @ref(tbl-summary-of-depression-and-self-esteem), and the summary of Rosenberg scores with their corresponding CES-D depression level is given in Table @ref(tbl-explicit-self-esteem).

# Discussion

The results suggest a significant relationship between depression and self-esteem. There was a mean score of `r summary_stats$Mean_CESD` in the CES-D, signifying moderate levels of depressive symptoms present in this sample. This effect size (η² = `r eta_squared_value`) shows a huge impact of depression level on overall CES-D scores. In regards to a relationship between age and depressive symptoms, a linear regression analysis was performed. This demonstrated that age `r ifelse(summary(lm_model)$coefficients[2, 4] < 0.05, "was", "was not")` a significant predictor of depression symptoms ($\beta$ = `r summary(lm_model)$coefficients[2, 1]`, *p* = `r summary(lm_model)$coefficients[2, 4]`). This demonstrates that `r ifelse(summary(lm_model)$coefficients[2, 4] < 0.05, "older individuals may be disposed to higher levels of depressive symptoms.", "age is not significantly predictive of depressive symptoms in this sample.")`. The correlation analysis showed a negative relationship between CES-D scores and Rosenberg Self-Esteem values (*r* = `r cor_test_estimate`, *p* = `r cor_test_p_value`, 95% CI [`r cor_test_conf_int[1]`, `r cor_test_conf_int[2]`]). This is consistent with prior research [@franck_implicit_2007; @yin_relationship_2022].
