...
#renaming RSES and CES-D variables
...
wave1_cohort1_clean <- wave1_cohort1 %>% #this gives errors, fix!!
  mutate(across(c(CESDQ1, CESDQ2, CESDQ3, CESDQ5, CESDQ6, CESDQ7, 
                  CESDQ9, CESDQ10,CESDQ11, CESDQ13, CESDQ14, CESDQ15, 
                  CESDQ17, CESDQ18, CESDQ19, CESDQ20)), ~ as.numeric(fct_recode(as.factor(.),
...
wave1_cohort1_clean <- wave1_cohort1_clean %>%
  mutate(across(c(CESDQ4, CESDQ8, CESDQ12, CESDQ16), ~ as.numeric(fct_recode(as.factor(.),
...
ggplot(wave1_cohort1_clean, aes(x = Demo_Q1, y = CESD_Total)) +
...
#linear regression
lm_model <- lm(CESD_Total ~ Demo_Q1, data = wave1_cohort1_clean)
summary(lm_model)
...
The results suggest a significant relationship between depression and self-esteem. There was a mean score of `r summary_stats$Mean_CESD`in the CES-D, signifying moderate levels of depressive symptoms present in this sample. The results of ANOVA (F = `r summary(anova_result)[[1]]$F value[1]`, *p* = `r summary(anova_result)[[1]]$Pr(>F)[1]`) demonstrated that higher levels of depression is correlated with severity of depression symptoms, udnerstandably as both come from the same CES-D data. In regards to a relationship between age and depressive symptoms, a linear regression analysis was performed. This demonstrated that age `r ifelse(summary(lm_model)$coefficients[2, 4] < 0.05, "was", "was not")` a significant predictor of depression symptoms (*β* = `r summary(lm_model)$coefficients[2, 1]`, *p* = `r summary(lm_model)$coefficients[2, 4]`). This demonstrates that `r ifelse(summary(lm_model)$coefficients[2, 4] < 0.05, "older individuals may be disposed to higher levels of depressive symptoms.", "age is not significantly predictive of depressive symptoms in this sample.")`
